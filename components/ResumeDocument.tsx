...
export default function ResumeDocument({ content, template, isTrial = false }: ResumeDocumentProps) {
  return (
    <div className={`resume-canvas-wrapper flex justify-center py-4 md:py-8 bg-slate-100 min-h-screen overflow-x-hidden print:p-0 print:bg-white print:block print:min-h-0 print:h-auto ${isTrial ? 'select-none' : ''}`}
         onContextMenu={(e) => e.preventDefault()}>
      <div className="relative group">
        {/* Anti-Inspect / Anti-Copy Shield */}
        <div className="absolute inset-0 z-[70] bg-transparent cursor-default print:hidden" 
             style={{ touchAction: 'none' }}
             onContextMenu={(e) => e.preventDefault()}
        />
        
        {/* Scaling Container for Mobile */}
        <div className="resume-scaler-container flex items-start justify-center overflow-hidden" 
             style={{ 
          ...
          <div className="resume-canvas shadow-2xl print:shadow-none bg-white relative origin-top"
               style={{ 
                 width: '210mm', 
                 height: '297mm',
                 transform: 'scale(calc((100vw - 2rem) / 794))', // 210mm is approx 794px
